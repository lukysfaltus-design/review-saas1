...
export async function PATCH(req, { params }) {
  ...
  const { name, google_review_url, owner_email, logo_url, instagram_url, facebook_url, website_url, accent_color } = body;
  if (!name || !google_review_url || !owner_email) {
    return NextResponse.json({ error: 'Vyplňte prosím povinná pole.' }, { status: 400 });
  }

  const db = supabaseAdmin();
  const { error } = await db.from('businesses').update({
    name, google_review_url, owner_email,
